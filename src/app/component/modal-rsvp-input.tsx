...
export default function ModalRsvpInput({
  ...
  const { name, status, total_guests, code, is_present } = data;
  let statusRead = "";
  if (is_present) {
    statusRead = "Datang";
  } else {
    ...
    >
      {is_present ? (
        <div className="flex flex-col">
          ...
          <div className="flex flex-col gap-3 text-white">
            ...
            <div className="flex justify-start text-left">
              <div className="w-1/2 lg:w-1/4">Nama</div>
              <div>{name}</div>
            </div>
            <div className="flex justify-start text-left">
              <div className="w-1/2 lg:w-1/4">Status</div>
              <div>{statusRead}</div>
            </div>
            <div className="flex justify-start text-left">
              <div className="w-1/2 lg:w-1/4">Jumlah Tamu</div>
              <div>{total_guests}</div>
            </div>
            <div className="flex justify-start text-left">
              <div className="w-1/2 lg:w-1/4">Kode Undangan</div>
        ...
        <>
          ...
          <div className="flex flex-col gap-3 text-white">
            <div>Semoga ketemu di lain kesempatan yaa.</div>
            <button
              onClick={onClose}
              className="px-3 py-2 mt-4 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 transition"
            >
